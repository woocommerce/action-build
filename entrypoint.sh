...
if [ -r "${WORKING_DIRECTORY}/.distignore" ]; then
  rsync -rc --include="$WORKING_DIRECTORY/node_modules/.bin/wc-e2e" --exclude-from="$WORKING_DIRECTORY/.distignore" "$WORKING_DIRECTORY/" "$DEST_PATH/" --delete --delete-excluded
else
