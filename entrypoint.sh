...
if [ -r "${WORKING_DIRECTORY}/.distignore" ]; then
  mkdir -p "$DEST_PATH/node_modules/.bin" && cp "${WORKING_DIRECTORY}/node_modules/.bin/wc-e2e" && rsync -rc --exclude-from="$WORKING_DIRECTORY/.distignore" "$WORKING_DIRECTORY/" "$DEST_PATH/"
else
